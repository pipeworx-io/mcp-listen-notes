...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Listen Notes');
}
...
const tools: McpToolExport['tools'] = [
  ...
  { name: 'podcast', description: 'Fetch full details for a single podcast by Listen Notes `id`. Returns title, description, publisher, language, total episodes, latest episode date, and image URL.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
  { name: 'episode', description: 'Fetch full details for a single podcast episode by Listen Notes `id`. Returns title, description, audio URL, duration, publish date, and parent podcast info.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
  { name: 'genres', description: 'Return the full list of Listen Notes podcast genre categories with genre id and name. Use genre_id values to filter `best_podcasts` or `search` by genre.', inputSchema: { type: 'object', properties: {} } },
  {
    name: 'best_podcasts',
    description: 'Fetch the Listen Notes curated list of top podcasts, optionally filtered by `genre_id`, `region`, or `sort`. Returns title, publisher, image URL, and Listen Notes id per podcast.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  { name: 'recommendations_for_podcast', description: 'Fetch podcast recommendations similar to a given podcast by Listen Notes `id`. Returns a list of similar podcasts with title, publisher, description, and image URL.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
  { name: 'recommendations_for_episode', description: 'Fetch episode recommendations similar to a given episode by Listen Notes `id`. Returns a list of similar episodes with title, audio URL, duration, and parent podcast info.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
];

async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const apiKey = (args._apiKey as string | undefined)?.trim();
  if (!apiKey) throw new Error('Listen Notes requires an API key: pass your key as the _apiKey argument (free at https://www.listennotes.com/api/dashboard/).');
  const get = async (path: string, params?: URLSearchParams) => {
    ...
    if (!res.ok) throw await httpError(res, 'Listen Notes');
    return res.json();
  };
  switch (name) {
    case 'search': {
